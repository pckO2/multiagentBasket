...
class PsychologicalAgent(BaseAgent):
    ...
    def summarize_chunkLocal(self, chunk):
        ...
        for p in chunk:
            summary = {
                ...
                "mental_toughness": p.get("psychological_profile", {}).get("mental_toughness"),
                "team_player": p.get("psychological_profile", {}).get("team_player"),
                "leadership_skills": p.get("psychological_profile", {}).get("leadership_skills"),
                "coach_feedback": p.get("coach_feedback", [])
            },
    ...
    def build_final_prompt(self, summaries):
        ...
        self.final_prompt  = "Eres un experto psicológico en baloncesto. Quiero que generes la alineación titular más efectiva posible basada en las información de los jugadores proporcionados."
        self.final_prompt  += " Quiero que priorices las habilidades de los jugadores basicas con la información proporcionada. En la alineación debes incluir 5 jugadores, uno para cada posición: Base, Escolta, Alero, Ala-Pívot y Pívot."
        ...
        self.final_prompt  += " Considera sus estado y perfil psicologico, la química del equipo y las dinámicas de juego al seleccionar la alineación."
